...
import { customButtonIconStyles, customButtonStyles } from '../../styles/CustomButtonStyles'
...
const Navbar = ({buttonFunction}:{buttonFunction:() => void}) => {
  return (
    <div className = 'bg-[var(--main-white)] py-[10px] pl-[0px] pr-[20px] flex flex-row justify-between w-full'>
        <Button className = {customButtonIconStyles({intent:"default" , disabled:false})} onClick = {buttonFunction}><img src = {IconMenu}/></Button>
        <a href = {Resume} target = '_blank' className = {customButtonStyles({intent:'default', disabled:false})}>
            Download CV
            <img src={IconFile} />
        </a>
    </div>
